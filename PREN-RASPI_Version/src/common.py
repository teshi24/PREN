...
import os
import time
from verification_interface import verfication_interface
...
def write_to_file(positions_json, i):
    file_name = "positions_config_{i}.json"
    #file_path = os.path.join('testdata', file_name)
    file_path = "/home/pi/PREN-main/json.json"
    with open(file_path, 'w') as file:
        json.dump(positions_json, file)
    verfication_interface.send_config(positions_json)
...
class Main:
    def main(self):
        while True:
            verfication_interface.start_run()
            ...
            positions_json = {'time': datetime.now().strftime('%Y-%m-%d %H:%M:%S'), 'config': positions}
...
# if dummies:
    # from Display.progress_bar import 
    # progress_bar_func = update_display
